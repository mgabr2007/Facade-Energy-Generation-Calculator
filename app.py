...
if st.button("Calculate Energy Generation"):
    ...
    if study_start_date >= study_end_date:
        ...
        if tmy_data is not None:
            ...
            # Retrieve PV module data
            sam_data = pvlib.pvsystem.retrieve_sam('SandiaMod')
            
            # List available modules if needed
            available_modules = list(sam_data.keys())
            st.write("Available modules:", available_modules)

            # Select a PV module (check if the module exists)
            module_name = 'Canadian_Solar_CS5P_220M__2009_'
            if module_name not in sam_data:
                st.error(f"Module '{module_name}' not found. Please select a different module.")
            else:
